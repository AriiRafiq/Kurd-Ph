document.addEventListener('DOMContentLoaded', () => {
	...
	const messageBoxes = [
        ...
        document.getElementById('messageBox1'),
        document.getElementById('messageBox2'),
        document.getElementById('messageBox3'),
        document.getElementById('messageBox4'),
        document.getElementById('messageBox5'),
        document.getElementById('messageBox6')
    ];

    // Function to show a specific message box
function showMessage(index, message) {
    if (index >= 0 && index < messageBoxes.length) {
        const messageBox = messageBoxes[index];
        messageBox.style.display = 'inline-block';
        messageBox.classList.remove('fade-out'); // Ensure the fade-out class is removed
        ...
        // Remove message box from DOM after fade-out animation completes
        messageBox.addEventListener('animationend', () => {
            if (messageBox.classList.contains('fade-out')) {
                messageBox.style.display = 'none';
                messageBox.classList.remove('fade-out'); // Reset class for next time
            }
        });
    }
}

// Show message boxes when 'instructionsButton' is clicked
document.getElementById('instructionsButton').addEventListener('click', () => {
    showMessage(0, "Click the title to clear all.");
    showMessage(1, "Enter the patient's age when weight is unknown.");
    showMessage(2, "Select the correct gender for your patient.");
    showMessage(3, "Click to calculate the assumed weight of the patient.");
    showMessage(4, "Enter the patient's weight when it is known.");
    showMessage(5, "Select the medication prescribed to your patient.");
    showMessage(6, "Select the dose of the syrup you have available.");
    sideMenu.classList.remove('open');
    menuButton.classList.remove('clicked');
});
